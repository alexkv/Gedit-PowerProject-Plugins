...
class Session(object):
	def __init__(self, name, files = None, path = ''):
...
class XMLSessionStore(SessionStore):
	def __init__(self):
		...
		self.filename = os.path.expanduser('~/.gnome2/gedit/powerproject/saved-sessions.xml')
		self.load()
	...
	def _dump_session(self, session):
		inner_xml = ''.join(['  <file path="%s"/>\n' % self._escape(filename)
							for filename in session.files])

		if session.get_path():
			inner_xml += '  <workingdirectory path="%s"/>\n' % self._escape(session.get_path())


		session_name = self._escape(session.name)
		return '<session name="%s">\n%s</session>\n' % (session_name, inner_xml)
	...
	def _expat_start_handler(self, tag, attr):
		if tag == 'workingdirectory':
			assert self._current_session is not None
			self._current_session.set_path(str(attr['path']))
		if tag == 'file':
			assert self._current_session is not None
